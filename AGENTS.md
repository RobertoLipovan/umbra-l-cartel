...
Cada vista empieza siempre **arriba del todo**. En una SPA el navegador no reinicia el scroll por su cuenta (como sí haría al cargar una página nueva), así que sin esto la vista nueva heredaba el scroll de la anterior y aparecía ya bajada — pasaba "a veces", solo al enlazar desde una zona scrolleada (p. ej. el botón "Interior" al final del lineup exterior, o el logo desde abajo). Lo hace `resetScroll()` en `router.js`, dentro de `mount()`, justo después de cambiar el contenido de `#app` y **antes** de `updateStageBanner()` (si no, la franja de escenario se calcularía con el scroll viejo y asomaría un instante).

El salto no se ve porque `wipeTransition()` llama a `mount()` con la máscara verde cubriendo la pantalla. Y tiene que ser instantáneo: `html` tiene `scroll-behavior: smooth` (`style.css`), con lo que un `window.scrollTo(0, 0)` normal se anima ~500 ms y se vería al retirar la máscara (se aterrizaría a mitad de recorrido). Detalle que costó encontrar, medido con Playwright:

- Poner `scroll-behavior: auto` inline en `<html>` **no basta**: si no se fuerza el recálculo de estilos, Chromium sigue leyendo el `smooth` de la hoja y anima igual (justo después de la llamada el scroll seguía en `772`, en vez de `0`). Hay que leer el valor calculado (`void getComputedStyle(root).scrollBehavior`) entre el cambio de estilo y el `scrollTo`.
- Alternativa válida: `window.scrollTo({ top: 0, behavior: 'instant' })`. Está implementado con el estilo inline + recálculo por ser el camino con más soporte entre navegadores.
- `document.scrollingElement.scrollTop = 0` también se anima (mismo `smooth`), o sea que no sirve.

Para reproducir/verificar: bajar al fondo de Exterior, enlazar a Interior y comprobar `window.scrollY === 0` al final (y, con una traza por frame, que ya es `0` en el mismo frame del cambio de vista, sin valores intermedios). Y que el ancla interna `#lineup` (sin `data-view`, ver la sección del router) sigue haciendo su scroll nativo: el reset solo afecta a los cambios de vista.
