...
`src/tilt.js` (`startCursorTilt()`, llamado desde `router.js` en cada `mount()`) inclina **`#app`** en 3D según la posición del cursor (`rotateX`/`rotateY`, `MAX_TILT_DEG = 2.5`). Necesita `perspective` en `html` y `transform-style: preserve-3d` en `#app` (`style.css`).
